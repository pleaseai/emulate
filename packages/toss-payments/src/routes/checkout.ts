...
import { escapeAttr, renderCheckoutPage } from '@emulators/core'
import {
  buildCard,
  formatPayment,
  generatePaymentKey,
...
export function checkoutRoutes(ctx: RouteContext): void {
  ...
  app.get('/checkout', (c) => {
    ...
    if (!clientKey) {
      return tossError(c, 400, 'INVALID_REQUEST', 'clientKey가 필요합니다.')
    }
  ...
  app.get('/checkout/approve', (c) => {
    ...
    if (!paymentKey) {
      return tossError(c, 400, 'INVALID_REQUEST', 'paymentKey가 필요합니다.')
    }
    ...
    if (!url.searchParams.has('orderId')) {
      url.searchParams.set('orderId', updated!.order_id)
    }
    if (!url.searchParams.has('amount')) {
      url.searchParams.set('amount', String(updated!.total_amount))
    }
