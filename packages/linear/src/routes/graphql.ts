...
export function graphqlRoutes(ctx: RouteContext): void {
  ...
  // core's app router has no `all`; register 405 for the non-POST methods explicitly.
  for (const register of [app.get, app.put, app.patch, app.delete] as const) {
    register.call(app, '/graphql', c =>
