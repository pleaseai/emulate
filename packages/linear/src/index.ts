...
function findOrganization(store: Store, ref: string | undefined) {
  const ls = getLinearStore(store)
  if (!ref) {
    return ls.organizations.all()[0] ?? null
  }
  return (
...
export function seedFromConfig(store: Store, baseUrl: string, config: LinearSeedConfig): void {
  ...
  for (const org of config.organizations ?? []) {
    ...
    if (ls.organizations.findOneBy('linear_id', linearId)) {
      continue
    }
  ...
  for (const user of config.users ?? []) {
    ...
    if (ls.users.findOneBy('linear_id', linearId)) {
      continue
    }
  ...
  for (const team of config.teams ?? []) {
    ...
    if (ls.teams.findOneBy('linear_id', linearId)) {
      continue
    }
  ...
  for (const state of config.workflow_states ?? []) {
    ...
    if (ls.workflowStates.findOneBy('linear_id', linearId)) {
      continue
    }
  ...
  for (const label of config.labels ?? []) {
    ...
    if (ls.labels.findOneBy('linear_id', linearId)) {
      continue
    }
  ...
  for (const project of config.projects ?? []) {
    ...
    if (ls.projects.findOneBy('linear_id', linearId)) {
      continue
    }
  ...
  for (const issue of config.issues ?? []) {
    ...
    if (ls.issues.findOneBy('linear_id', linearId)) {
      continue
    }
