...
import type { KakaoStore } from '../store.js'
import { extractBearer, kapiError, parseKakaoBody } from '../helpers.js'
...
export function userRoutes(ctx: RouteContext): void {
  ...
  const me = (c: Context) => {
    ...
    if (!auth) {
      return kapiError(c, 401, 'this access token does not exist', -401)
    }
  ...
  app.post('/v1/user/unlink', (c) => {
    ...
    for (const t of ks().tokens.all()) {
      if (t.user_id === userId) {
        ks().tokens.update(t.id, { active: false })
      }
    }
