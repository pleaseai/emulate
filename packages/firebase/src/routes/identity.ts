...
async function handleSignUp(c: Context, store: import("@emulators/core").Store, fs: () => FirebaseStore) {
  ...
  if (!projectId) return apiKeyError(c);
...
async function handleSignIn(c: Context, store: import("@emulators/core").Store, fs: () => FirebaseStore) {
  ...
  if (!user) return identityError(c, "EMAIL_NOT_FOUND");
  if (user.password !== password) return identityError(c, "INVALID_PASSWORD");
  ...
  if (fresh.display_name) response.displayName = fresh.display_name;
...
async function handleLookup(c: Context, store: import("@emulators/core").Store, fs: () => FirebaseStore) {
  ...
  if (!user) return identityError(c, "INVALID_ID_TOKEN");
  ...
  if (user.display_name) userInfo.displayName = user.display_name;
  if (user.password) userInfo.passwordHash = "emulator:" + user.password;
...
async function handleUpdate(c: Context, store: import("@emulators/core").Store, fs: () => FirebaseStore) {
  ...
  if (deleteAttribute.includes("DISPLAY_NAME")) patch.display_name = null;
  ...
  if (fresh.email) response.email = fresh.email;
  if (fresh.display_name) response.displayName = fresh.display_name;
  if (fresh.password) response.passwordHash = "emulator:" + fresh.password;
...
async function handleDelete(c: Context, store: import("@emulators/core").Store, fs: () => FirebaseStore) {
  ...
  for (const t of s.tokens.all()) {
    if (t.local_id === user.local_id) s.tokens.delete(t.id);
  }
...
async function handleSendOobCode(c: Context, store: import("@emulators/core").Store, fs: () => FirebaseStore) {
  ...
  if (!email) return identityError(c, "MISSING_EMAIL");
  if (requestType === "PASSWORD_RESET" && !user) return identityError(c, "EMAIL_NOT_FOUND");
