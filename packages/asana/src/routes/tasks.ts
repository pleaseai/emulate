...
export function taskRoutes({ app, store, baseUrl }: RouteContext): void {
  ...
  app.get('/api/1.0/tasks', (c) => {
    ...
    if (projectGid) {
      ...
      const rels = as()
        .taskProjects
        .findBy('project_gid', section.project_gid)
  ...
  app.post('/api/1.0/tasks', async (c) => {
    ...
    if (!body.name && body.name !== '') {
      return asanaError(c, 400, 'name: Missing input')
    }
    ...
    if (membershipData) {
      for (const m of membershipData) {
        ...
        if (existing) {
          if (m.section) {
            as().taskProjects.update(existing.id, { section_gid: m.section })
          }
        }
        else {
  ...
  app.get('/api/1.0/tasks/:task_gid', (c) => {
    ...
    if (!task) {
      return asanaError(c, 404, 'task: Not Found')
    }
  ...
  app.put('/api/1.0/tasks/:task_gid', async (c) => {
    ...
    if (body.assignee !== undefined) {
      updates.assignee_gid = body.assignee as string | null
    }
    ...
    if (body.due_on !== undefined) {
      updates.due_on = body.due_on as string | null
    }
    if (body.due_at !== undefined) {
      updates.due_at = body.due_at as string | null
    }
    if (body.start_on !== undefined) {
      updates.start_on = body.start_on as string | null
    }
    if (body.notes !== undefined) {
      updates.notes = body.notes as string
    }
    if (body.html_notes !== undefined) {
      updates.html_notes = body.html_notes as string
    }
    if (body.resource_subtype !== undefined) {
      updates.resource_subtype = body.resource_subtype as AsanaTask['resource_subtype']
    }
  ...
  app.delete('/api/1.0/tasks/:task_gid', (c) => {
    ...
    for (const tp of as().taskProjects.findBy('task_gid', gid)) {
      as().taskProjects.delete(tp.id)
    }
    for (const tt of as().taskTags.findBy('task_gid', gid)) {
      as().taskTags.delete(tt.id)
    }
    for (const td of as().taskDependencies.findBy('task_gid', gid)) {
      as().taskDependencies.delete(td.id)
    }
    for (const td of as().taskDependencies.findBy('dependency_gid', gid)) {
      as().taskDependencies.delete(td.id)
    }
    for (const story of as().stories.findBy('task_gid', gid)) {
  ...
  app.post('/api/1.0/tasks/:task_gid/stories', async (c) => {
    ...
    const body = await parseAsanaBody(c)
    if (!body.text) {
      return asanaError(c, 400, 'text: Missing input')
    }
  ...
  app.post('/api/1.0/tasks/:task_gid/addProject', async (c) => {
    ...
    if (!projectGid) {
      return asanaError(c, 400, 'project: Missing input')
    }
  ...
  app.post('/api/1.0/tasks/:task_gid/addTag', async (c) => {
    ...
    if (!tagGid) {
      return asanaError(c, 400, 'tag: Missing input')
    }

    const tag = as().tags.findOneBy('gid', tagGid)
    if (!tag) {
      return asanaError(c, 404, 'tag: Not Found')
    }
  ...
  app.post('/api/1.0/tasks/:task_gid/addDependencies', async (c) => {
    ...
    if (!dependencies || !Array.isArray(dependencies)) {
      return asanaError(c, 400, 'dependencies: Missing input')
    }
  ...
  app.post('/api/1.0/tasks/:task_gid/removeDependencies', async (c) => {
    ...
    for (const depGid of dependencies) {
      ...
      if (rel) {
        as().taskDependencies.delete(rel.id)
      }
    }
  ...
  app.post('/api/1.0/tasks/:task_gid/addFollowers', async (c) => {
    ...
    if (!followers || !Array.isArray(followers)) {
      return asanaError(c, 400, 'followers: Missing input')
    }
