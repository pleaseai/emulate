...
export function projectRoutes({ app, store, baseUrl }: RouteContext): void {
  ...
  app.get('/api/1.0/projects', (c) => {
    ...
    if (teamGid) {
      projects = projects.filter(p => p.team_gid === teamGid)
    }
  ...
  app.put('/api/1.0/projects/:project_gid', async (c) => {
    ...
    if (body.archived !== undefined) {
      updates.archived = body.archived as boolean
    }
    if (body.privacy_setting !== undefined) {
      updates.privacy_setting = body.privacy_setting as AsanaProject['privacy_setting']
    }
    if (body.default_view !== undefined) {
      updates.default_view = body.default_view as AsanaProject['default_view']
    }
    if (body.owner !== undefined) {
      updates.owner_gid = body.owner as string
    }
    if (body.team !== undefined) {
      updates.team_gid = body.team as string
    }
