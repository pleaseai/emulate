...
export function webhookRoutes({ app, store, baseUrl }: RouteContext): void {
  ...
  app.get('/api/1.0/webhooks', (c) => {
    ...
    if (resourceGid) {
      webhooks = webhooks.filter(w => w.resource_gid === resourceGid)
    }
  ...
  app.post('/api/1.0/webhooks', async (c) => {
    ...
    if (!body.resource) {
      return asanaError(c, 400, 'resource: Missing input')
    }
    if (!body.target) {
      return asanaError(c, 400, 'target: Missing input')
    }
  ...
  app.get('/api/1.0/webhooks/:webhook_gid', (c) => {
    ...
    if (!webhook) {
      return asanaError(c, 404, 'webhook: Not Found')
    }
  ...
  app.put('/api/1.0/webhooks/:webhook_gid', async (c) => {
    ...
    if (body.active !== undefined) {
      updates.active = body.active as boolean
    }
