...
import { asanaData, asanaError, formatStory, parseAsanaBody } from '../helpers.js'
...
export function storyRoutes({ app, store }: RouteContext): void {
  ...
  app.get('/api/1.0/stories/:story_gid', (c) => {
    ...
    if (!story) {
      return asanaError(c, 404, 'story: Not Found')
    }
  ...
  app.put('/api/1.0/stories/:story_gid', async (c) => {
    ...
    if (!story.is_editable) {
      return asanaError(c, 403, 'story: Not editable')
    }
    ...
    if (body.html_text !== undefined) {
      updates.html_text = body.html_text as string
    }
